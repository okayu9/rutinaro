...
3. **IndexedDB ストレージ** (`src/lib/storage/indexeddb.ts`)
...
6. **プレビューコンポーネント** (`src/lib/components/PreviewPane.svelte`)
...
## 5. カレンダー画面
1. **月グリッドコンポーネント** (`src/lib/components/calendar/`)
...
2. **M2: ルーティン編集体験 (Week 2-3)**
   - フォーム、RRULE 処理、プレビュー機能。
3. **M3: カレンダー & 例外処理 (Week 4)**
   - 月グリッド、EXDATE/RDATE 操作、祝日ポリシー適用。
4. **M4: 設定・バックアップ・PWA (Week 5)**
   - ICS インポート、バックアップ、Service Worker。
5. **M5: テスト整備 & リリース (Week 6)**
