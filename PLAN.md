...
本計画は、仕様ドキュメントの要件を満たすための作業をフェーズ別・モジュール別に詳細化したものである。各タスクは、成果物、主担当モジュール、完了条件を明確にしている。
...
- [ ] **2.3 IndexedDB ストレージ** (`src/lib/storage/indexeddb.ts`)
...
- [ ] **4.6 プレビューコンポーネント** (`src/lib/components/PreviewPane.svelte`)
...
## 5. カレンダー画面
- [ ] **5.1 月グリッドコンポーネント** (`src/lib/components/calendar/`)
...
- [ ] **11.2 M2: ルーティン編集体験 (Week 2-3)**
   - フォーム、RRULE 処理、プレビュー機能。
- [ ] **11.3 M3: カレンダー & 例外処理 (Week 4)**
   - 月グリッド、EXDATE/RDATE 操作、祝日ポリシー適用。
- [ ] **11.4 M4: 設定・バックアップ・PWA (Week 5)**
   - ICS インポート、バックアップ、Service Worker。
- [ ] **11.5 M5: テスト整備 & リリース (Week 6)**
